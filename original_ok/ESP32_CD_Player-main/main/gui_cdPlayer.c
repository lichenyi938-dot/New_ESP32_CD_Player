...
// 如果你的工程已在 CMakeLists 里设置了 LV_CONF_INCLUDE_SIMPLE/LV_CONF_PATH，
// 这两行即使重复也不会出问题，是安全的双保险。
...
#include "lvgl.h"
#include "lvgl_compat.h"    // 兼容头：提供 LV_ALIGN_*/LV_SYMBOL_* 等在 v8 的等价实现
// ============================================================
...
void gui_player_init(void)
{
    ...
    gui_setTrackTitle("What title long long long long long",
                      "Who know long long long long long");
    ...
    lv_bar_set_range(bar_meterLeft, 35, 90);
    ...
    lv_chart_set_update_mode(chart_left, LV_CHART_UPDATE_MODE_SHIFT);
    lv_chart_set_point_count(chart_left, size_oscilloscopeWidth);
    lv_chart_set_range(chart_left, LV_CHART_AXIS_PRIMARY_Y, -maxPoint, maxPoint);
    ...
    if ((title != NULL) && (strcmp(oldTitle, title) != 0))
    {
        ...
        if (performer) {
            for (int i = 0; i < (int)strlen(performer); i++) {
                if (performer[i] != ' ') { noPerformer = false; break; }
            }
        }
...
void gui_setProgress(uint32_t current, uint32_t total)
{
    ...
    static uint32_t oldTotal   = 0;

    if (oldCurrent == current && oldTotal == total) return;
    oldCurrent = current;
    oldTotal   = total;

    if (total == 0) total = 1234; // 防止除零
...
void gui_setVolume(int vol)
{
    ...
    oldVol = vol;
    // 兼容层里把 lv_label_set_text 重定向到 lv_label_set_text_fmt，因此可以直接用格式化
